...
$h1_custom=$row_detail['h1'];
	...
	$sql = "select * from #_product where hienthi=1 and id_list = 7  order by stt asc";
		$d->query($sql);
		$product_datnen = $d->result_array();

	
		

	// san pham moi nhat dat nen
		$d->reset();
		$sql="select table_product.tenkhongdau AS tenkhongdaup,table_product_list.tenkhongdau AS tenkhongdaul,gia,table_product.ten as tenp,diachi,table_product.thumb as thumbp
		 from table_product,table_product_list where table_product.hienthi =1 and table_product.spbc > 0 and table_product.id_list = table_product_list.id and table_product.id_list = 7
		 order by table_product.stt,table_product.id 
		  limit 0,3";
	$d->query($sql);
	$r_product_newdatnen=$d->result_array();
	...
	$maxR=6;
	$maxP=5;
	$paging=paging_home($product_datnen, $url, $curPage, $maxR, $maxP);
	$product_datnen = $paging['source'];	
