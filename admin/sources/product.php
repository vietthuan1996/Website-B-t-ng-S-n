...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 300, 200, _upload_product,$file_name,1);		
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
		...
		$data['kieu'] = $_POST['kieu'];
		$data['tinhtrang'] = $_POST['tinhtrang'];	
		$data['dientich'] = $_POST['dientich'];
		$data['phongtam'] = $_POST['phongtam'];	
		if($data['phongtam'] < 0)
			$data['phongtam'] = 0;
		$data['phongngu'] = $_POST['phongngu'];	
		if($data['phongngu'] < 0)
			$data['phongngu'] = 0;
		$data['nhadexe'] = $_POST['nhadexe'];
			if($data['nhadexe'] < 0)
			$data['nhadexe'] = 0;	
		$data['tieude1'] = $_POST['tieude1'];	
		$data['tieude2'] = $_POST['tieude2'];	
		$data['tieude3'] = $_POST['tieude3'];	
		$data['chitiet1'] = $_POST['chitiet1'];	
		$data['chitiet2'] = $_POST['chitiet2'];	
		$data['chitiet3'] = $_POST['chitiet3'];		
...
function save_list(){
	...
	if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_name)){
			$data['photo'] = $photo;		
			$data['thumb'] = create_thumb($data['photo'], 300, 200, _upload_product,$file_name,2);		
		}				
		$data['ten'] = $_POST['ten'];
		$data['tenkhongdau'] = changeTitle($_POST['ten']);
		$data['title'] = $_POST['title'];
...
function save_photo(){
	...
	if($id){ // cap nhat
			...
			if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_product,$file_name)){
				$data['photo'] = $photo;
				$data['thumb'] = create_thumb($data['photo'], 300, 200, _upload_product,$file_name.$i,1);
				$d->setTable('product_hinhanh');
				$d->setWhere('id', $id);
				$d->select();
				if($d->num_rows()>0){
