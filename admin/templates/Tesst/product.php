...
switch($act){
	...
	case "man_list_photo":
		get_photos_list();
		$template = "product/photos_list";
		break;
	case "add_list_photo":
		$template = "product/photo_list_add";
		break;
	case "edit_list_photo":
		get_photo_list();
		$template = "product/photo_list_edit";
		break;
	case "save_list_photo":
		save_list_photo();
		break;
	case "delete_list_photo":
		delete_list_photo();
		break;
	#============================================================
	case "duyetbl":
...
function save_item(){
	...
	if($id){
		...
		if($photo = upload_image("file", 'jpg|png|gif|JPG|jpeg|JPEG', _upload_product,$file_name)){
			$data['photo'] = $photo;	
			$data['thumb'] = create_thumb($data['photo'], 153, 123, _upload_product,$file_name,2);		
			$d->setTable('product');
			$d->setWhere('id', $id);
			$d->select();
			if($d->num_rows()>0){
...
function get_photos_list(){
	...
	{
	$id_up = $_REQUEST['hienthi'];
	$sql_sp = "SELECT id,hienthi FROM table_list_hinhanh where id='".$id_up."' ";
	$d->query($sql_sp);
	$cats= $d->result_array();
	$hienthi=$cats[0]['hienthi'];
	if($hienthi==0)
	{
	$sqlUPDATE_ORDER = "UPDATE table_list_hinhanh SET hienthi =1 WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}
	else
	{
	$sqlUPDATE_ORDER = "UPDATE table_list_hinhanh SET hienthi =0  WHERE  id = ".$id_up."";
	$resultUPDATE_ORDER = mysql_query($sqlUPDATE_ORDER) or die("Not query sqlUPDATE_ORDER");
	}	
	}
	...
	$sql = "select * from #_list_hinhanh where ( id_photo = '".$_REQUEST['idc']."' OR '".$_REQUEST['idc']."'=0  ) order by stt,id desc ";			
	$d->query($sql);
	$items = $d->result_array();
	
	$curPage = isset($_GET['curPage']) ? $_GET['curPage'] : 1;
	$url="index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."";
	$maxR=10;
	$maxP=4;
	$paging=paging($items, $url, $curPage, $maxR, $maxP);
	$items=$paging['source'];
...
function get_photo_list(){
	...
	$id = isset($_GET['id']) ? themdau($_GET['id']) : "";
	if(!$id)
		transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
	
	$d->setTable('list_hinhanh');
	$d->setWhere('com', 'product');
	$d->setWhere('id', $id);
	$d->select();
	if($d->num_rows()==0) transfer("Dữ liệu không có thực", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
	$item = $d->fetch_array();
	$d->reset();
}

function save_list_photo(){
	global $d;
	$file_name=fns_Rand_digit(0,9,10);
	if(empty($_POST)) transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
	...
	if($id){ // cap nhat
			...
			$d->setTable('list_hinhanh');
			$d->setWhere('id', $id);
			if(!$d->update($data)) transfer("Cập nhật dữ liệu bị lỗi", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
			redirect("index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
				...
				if($photo = upload_image("file".$i, 'jpg|png|gif|JPG|jpeg|Jpg|JPEG', _upload_product,$file_name.$i))
					{
						...
						$d->setTable('list_hinhanh');
						if(!$d->insert($data)) transfer("Lưu dữ liệu bị lỗi", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
					}
			}
			redirect("index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
	}
...
function delete_list_photo(){
	...
	if(isset($_GET['id'])){
		...
		if($d->delete())
			...
			transfer("Xóa dữ liệu bị lỗi", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
	}else transfer("Không nhận được dữ liệu", "index.php?com=product&act=man_list_photo&idc=".$_REQUEST['idc']."");
}
